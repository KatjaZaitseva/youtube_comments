...
<!-- I can't understand what this operation does: dict_output <- cbind(dict_output, docvars(cleaned_comments)) -->
...
dict_output %>%
  ...
  ggplot2::annotate(geom = "label", x = mean_dict + 0.75, y = Inf, 
...
## Validation of Unweighted Sentiment Analysis
...
sample_comments_sentiment_no_scores <- sample_comments_sentiment %>% select(c(doc_id, textOriginal))
write.csv(sample_comments_sentiment_no_scores,"sample_comments_sentiment_no_scores.csv")
```
...
# save output to file
save(topicmodels_2, topicmodels_3, topicmodels_4, topicmodels_5, topicmodels_6, topicmodels_7, topicmodels_8, topicmodels_9, topicmodels_10,
...
```{r plot-topic-model, fig.cap = "Keyword Probabilities per Topic"}
...
lda_matrix <- tidy(topicmodels_3, matrix = "beta")
