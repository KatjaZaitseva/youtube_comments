...
```{r cut dataframe}
comments.cut <- comments %>% select(c(id, textOriginal, likeCount, parentId))
head(comments.cut$textOriginal)
```
...
urls.n <- sum(str_detect(comments.cut$textOriginal, "(http|ftp|https)://\\S+"))
mentions.n <- sum(str_detect(comments.cut$textOriginal, "(^|\\s)@\\w+"))
...
textCleaned <- comments.cut$textOriginal %>% 
  ...
  str_replace_all("(^|\\s)(@|#|https|http|ftp?://)\\S+", " ") %>% 
  #remove numbers
  str_replace_all("[0-9]+", " ") %>% 
