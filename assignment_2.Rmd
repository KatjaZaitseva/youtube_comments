...
To verify this finding, we carry out a dictionary-based sentiment analysis. For this, we use the underlying workings of the `quanteda.sentiment` package as described by \citet{Benoit2019} and \citet{Benoit2021}. As we are not aware of any dictionary dedicated to social media data, we make a few adaptions to the \emph{data\_dictionary\_LSD2015} dictionary. We remove, for example, the term \emph{demis*} from the list with negative word patterns as demisexuality\{Demisexuality is an identity falling under the asexual umbrella.} would be allocated a negative sentiment otherwise. Despite the need for alterations, however, the used dictionary has the clear advantage of including negations. This means that our analysis will account for our negation bigrams and recognise that terms such as 'not good' have a negative sentiment. The procedure is applied to the cleaned tokenised comments. To determine the global sentiment of the comments, we calculate the mean of the obtained sentiments scores. The code for this is taken from \citet{Benoit2019}.
...
As the dictionary-based sentiment scores are not normalised to be between -1 and 1, the results are scaled somewhat differently than the ones given by the `vader` package. Nonetheless, a global sentiment score of 0.5 further supports the finding that the overall sentiment of the comments is positive.
...
Aside from investigating how the audience perceived the video emotionally, we seek to understand what topics prevailed in the comment section. For this, we use the unsupervised method topic modelling. <!-- not sure this is needed: A DFM has one row per observation (or comment in our case) and one column per word. Accordingly, at the intersection of row i and column j is the value 1 if the word j is included in the comment i and the value 0 if it is not included. -->, in preparation of which we further wrangle the already cleaned comments. This includes the creation of a document-feature matrix (DFM) and the removal of low and high frequency words. More precisely, we remove words that appear in more than 10% of all comments and retain only the top 20% of the most frequent words. We eventually remove all empty rows as these would create issues when building the topic model. The code for this procedure is adapted from \citet{QuantedaTopicmodel}.
...
Before fitting the topic model, we need to determine the number of $k$, that is, the number of topics in the comments. To this end, we compute different metrics to help us identifying the optimal number of topics. The code for this is taken from \citet{Puschmann2022}.
...
According to Figure \@ref(fig:topic-numbers-plot), there is no straightforward answer to the question regarding the optimal number of topics. Whereas the metrics based on Griffiths2004 and Arun2010 gradually improve and thus support $k = 15$, CaoJuan2009 and Deveaud2014's measures suggest that $k = 3$ and $k = 10$ fit our data well. Given these inconclusive results, we now fit various topic models with different numbers for $k$ (range 2 to 10). 
...
Next, we look at the output of the different topic models to manually check which number of topics makes most sense semantically and is most coherent (output of models with $k > 5$ are omitted).
...
Based on the output, four topics seem to be the most coherent. However, it should be noted that the topics are not clearly distinguished in any of the inspected models. Upon identifying the number of topics, we plot the keywords and their probabilities for $k = 4$. The code for the visualisation is adopted from \citet{Silge2022} and \citet{Silge2018}.
...
The aim of the validation of the sentiment analysis is to test how accurately the dictionary approach detects the sentiment of the comments. For this, we loosely follow the procedure described by \citet{SentimentValidation}. We start the validation process by randomly sampling 100 comments. Each of the researchers in this paper (3 in total) then reads the sampled comments and classifies them as either positive, negative or neutral. When coding the comments, the researchers are unaware of the results of the dictionary-based sentiment analysis .
...
Upon manually coding the data, we import the coded comments and merge them into a data frame.
...
We then transform the data frame and calculate the intercoder reliability using Krippendorff's $\alpha$. 
...
# convert to matrix
sentiment_validation_m <- sentiment_validation_t %>% as.matrix()

# compute intercoder reliability
sentiment_validation_m %>% kripp.alpha(method = "ordinal")

# compute pairwise intercoder reliability
sentiment_validation_m[1:2, ] %>% kripp.alpha(method = "ordinal")
sentiment_validation_m[2:3, ] %>% kripp.alpha(method = "ordinal")
sentiment_validation_m[-2, ] %>% kripp.alpha(method = "ordinal")
```

As we can see above, Krippendorff’s $\alpha$ is 0.77, with the pairwise intercoder reliability ranging from 0.69 to 0.87. Using a majority vote, we now add a new column to the data frame indicating the dominant sentiment of the manual coding.
...
Next, in accordance with the recommendations made by \citep{VaderGit}, each numerical sentiment score from the unweighted sentiment analysis is assigned a sentiment category (positive, negative, neutral). 
...
```
We then transform the data frame with the results of the dictionary approach and merge it with the results of the manual coding.
...
# convert to matrix
sentiment_merged_m <- sentiment_merged %>% as.matrix()

# compute agreement between manual coding and dictionary approach
sentiment_merged_m %>% kripp.alpha(method = "ordinal")
```

As is apparent in the output, Krippendorff's $\alpha$ of 0.37 is rather low. To investigate any misclassifications, we therefore take a look at a confusion matrix.
