...
After having accounted for negation words, we remove them along with other stopwords. For this, we use the 'smart' dictionary, because there are inconsistencies in other popular dictionaries. For example, whilst 'can't' and 'cannot' are removed with the 'en' dictionary, its counterpart 'can' is not, which would lead to skewed results. We moreover customise the stopwords by removing the words 'thank' and 'thanks' from the list. Again, we use the padding statement to remember the initial position of each word in a comment.
...
Next, we check for the most common collocations (i.e., n-grams that occur more often than by chance). As part of this procedure, we set the minimum number of occurrences to 10 and limit the n-grams to bigrams. Due to our consistent usage of the padding statement, the function will only generate a bigram when two words were adjacent in the original comment. Our code follows the example given by \citet{QuantedaCompound}.
...
To verify this finding, we carry out a dictionary-based sentiment analysis. For this, we use the underlying workings of the `quanteda.sentiment` package as described by \citet{Benoit2019} and \citet{Benoit2021}. As we are not aware of any dictionary dedicated to social media data, we make a few adaptions to the \emph{data\_dictionary\_LSD2015} dictionary. We remove, for example, the term \emph{demis*} from the list with negative word patterns as demisexuality\footnote{Demisexuality is an identity falling under the asexual umbrella.} would be allocated a negative sentiment otherwise. Despite the need for alterations, however, the used dictionary has a clear advantage: It includes negations. This means that our analysis will account for our negation bigrams and recognise that terms such as 'not good' have a negative sentiment. The procedure is furthermore applied to the cleaned tokenised comments. To determine the global sentiment of the comments, we calculate the mean of the obtained sentiments scores. The code for this is taken from \citet{Benoit2019}.
...
As the dictionary-based sentiment scores are not normalised to be between -1 and 1, the results are scaled somewhat differently than the ones given by the `vader` package. Nonetheless, a global sentiment score of 0.5 also suggests that the overall sentiment of the comments is positive.

Following the calculation of the sentiment scores, we plot the results of the dictionary-based sentiment analysis.
...
Aside from investigating how the audience perceived the video emotionally, we seek to understand what topics prevailed in the comment section using the unsupervised method topic modelling, <!-- not sure this is needed: A DFM has one row per observation (or comment in our case) and one column per word. Accordingly, at the intersection of row i and column j is the value 1 if the word j is included in the comment i and the value 0 if it is not included. -->In preparation for this, we further wrangle the already cleaned comments. This includes the creation of a document-feature matrix (DFM) and the removal of low and high frequency words. More precisely, we retain only the top 20% of the most frequent words and remove all words that appear in more than 10% of all comments. We eventually remove all empty rows as these would create issues when building the topic model. The code for this procedure is adapted from \citet{QuantedaTopicmodel}.
...
Next, we look at the output of the different topic models to manually check which number of topics makes most sense semantically (output of models with $k > 6$ are omitted).
...
To validate the results of our analyses, we now turn to our robustness checks.

## Validation of Sentiment Analyis
...
First, we randomly sample 100 comments. Each of the researchers in this paper, that is, 3 in total, then reads the sampled comments and classifies them as either positive, negative or neutral. When coding the comments, the researchers are unaware of the results of the dictionary-based sentiment analysis.
...
Next, in accordance with the recommendations made by \citet{VaderGit}, each numerical sentiment score from the unweighted sentiment analysis is assigned one of the following sentiment categories: positive, negative or neutral. 
...
```
After that, we transpose the data frame with the results of the dictionary approach and merge it with the dominant results of the manual coding.
...
As is apparent in the output, Krippendorff's $\alpha$ is 0.37, which is rather low but higher than the values obtained by \citet{SentimentValidation} when comparing the precision of different dictionaries. To investigate any misclassifications, we next take a look at a confusion matrix.
...
The table reveals that most comments (i.e., 59) are correctly classified. We can further see that when misclassifications occur, this mostly tends to happen between neutral and positive sentiments. What stands out in the table is that two comments were classified as positive even though the coders deemed them as negative, and two comments were classified as negative even though the coders deemed them as positive. To further investigate this, we take a closer look at the misclassified comments. Although this procedure is undertaken for all misclassified comments, we exemplify our course of action with comment number 97832.
...
As shown above, the dictionary-based approach assigned comment number 97832 a positive sentiment. This is becasue the terms \emph{love} and \emph{uphold*} are included as positive word patterns in the used dictionary. The manual coders, on the other hand, classified the comment as negative\footnote{To be precise, two researchers coded it as negative, and one coded it as neutral.}. Even though the scores assigned by the dictionary approach contradict the manually coded sentiments, this appears to be not a result of a poorly constructed dictionary but more so an issue of the employed method which largely neglects the context of words.

## Validation of Topic Modelling

https://medium.com/pew-research-center-decoded/interpreting-and-validating-topic-models-ff8f67e07a32
