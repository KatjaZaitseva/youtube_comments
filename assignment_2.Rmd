...
To address our research aims, we use a combination of web scraping, sentiment analysis and topic modelling. We start our project by scraping the comments via the Youtube API. We then clean and explore the data. For this, we use a range of methods, namely subsetting, noise removal, negation bigrams, stopword removal, collocations, lemmatisation and the removal of irrelevant high frequency words. After that, we conduct a dictionary-based sentiment analysis to quantify how the majority of viewers felt about the video (i.e., positively or negatively). This process consists of two steps: First, we carry out a sentiment analysis with the cleaned comments. Second, we account for any differences in the comments' popularity by weighting each comment according to their like number. Finally, to explore the key themes in the comments, we employ topic modelling.
...
For comparability purposes, we also use \cites{RiederTools} web tool to extract the comments, which results in 218,848 observations. What is striking about these results is that none of the tested packages and tools are able to extract all Youtube comments. Since we conduct our analysis in R, we opt for the `tuber` package as it not only parses about 30,000 comments more than the `vosonSML` package but also is more transparent regarding the issue with the number of returned comments \citep[see e.g.,][]{IssuesTuber}. The exact code we used to collect our data on the 16th May 2022 is shown below.
...
<!-- She mentioned 'make sure to explain in detail how you have sampled your items (random sample, stratified sample...) and why this sampling technique was the best one for your research question', even though we do sampling only here, better to explain why specifically this technique -->

To ensure our results are reproducible, we use the set seed function. We then randomly sample 1,000 comments from our data set. We used random sampling without replacement because we did not have the availability of other variables about users to take advantage of systematic or stratified sampling procedures. As the `vader` package is applied to textual and not tokenised data, we use the uncleaned column \emph{textOriginal} for this analysis. Upon obtaining the results from the `vader` package, we calculate the average sentiment across all comments.
...
To verify this finding, we carry out a dictionary-based sentiment analysis. For this, we use the underlying workings of the `quanteda.sentiment` package as described by \citet{Benoit2019, Benoit2021}. As we are not aware of any dictionary dedicated to social media data, we make a few adaptions to the \emph{data\_dictionary\_LSD2015} dictionary. We remove, for example, the term \emph{demis*} from the list with negative word patterns as demisexuality\footnote{Demisexuality is an identity falling under the asexual umbrella \citep{Antonsen2020}.} would be allocated a negative sentiment otherwise. However, despite the need for alterations, the used dictionary has a clear advantage: It includes negations. This means that our analysis will account for our negation bigrams and recognise that terms such as ‘not good' have a negative sentiment. 

The dictionary-based sentiment analysis is applied to the cleaned tokenised comments. To determine the global sentiment of the comments, we calculate the mean of the obtained sentiments scores. The code for this is taken from \citet{Benoit2019}.
...
Prior to fitting the model, we determine the parameter $k$, that is, the number of topics in the comments. For this, we compute different metrics to help us identify the optimal number of topics. The code for this is taken from \citet{Puschmann2022}.
...
As shown above, the dictionary-based approach assigns comment number 97,832 a positive sentiment. This is because the terms \emph{love} and \emph{uphold*} are included as positive word patterns in the used dictionary. The manual coders, conversely, classify the comment as negative\footnote{To be precise, two researchers coded it as negative, and one coded it as neutral. The overall manual sentiment was therefore negative.}, because it invalidates Jaiden's experiences and is generally condescending. However, although the scores assigned by the dictionary approach contradict the manually coded sentiments, the inspection of the word patterns suggests that this is not a result of a poorly constructed dictionary but more so an issue of the employed method which largely neglects the context of words.
...
As for the second topic, which we initially assigned the name \emph{Opinion}, there is a consensus among coders that all comments scoring high on this topic can be classified as spam. It is moreover undisputed that the topic contains unrelated messages (e.g., Christian and Islamic prayers), trolling (e.g., about 25 comments repeat ‘not being straight is bad’ over and over again), and non-English (mostly Spanish) comments. This is a particularly interesting finding given that the keywords suggested a completely different topic interpretation. In light of these results, we change the topic name to \emph{Spam}.
...
<!--Describe your results and conclusion concisely. That means: elaborate on meaningful results
and not on technical issues. We want an answer to the research question, and nog a technical
elaboration (what model performs best under which metrics, SE, failures, etc).-->

The video \citet{JaidenAnimationsVideo} introduced the audience to the topic of aromanticism and asexuality, in which the creator of the video also shared her personal experience. The first question that arises is how this video has been responded to. Of course, one can read the top of the first comments and try to draw conclusions as to how the public reacted to it, but when there are more than 100,000 comments, just a few comments cannot reveal the whole picture. This is why special algorithms are used for such big data analysis, which help to generalise the conclusions to the entire audience. In this work, 180,743 comments were obtained from the time the video was published (20 March 2022) until the 16th May 2022. After the text processing step, namely removing comments that are responses to other comments, clearing the text of noise, tokenisation, adding negation bigram and collocations, removing stopwords and lemmatisation, we were left with 118,342 comments. A sentiment analysis was carried out, which assigned the score to each comment and revealed that comments in general are positive with the total score 0.5 which is significantly more than zero. In addition, sentiment analysis was adapted to the specifics of Youtube, where there may be few comments, but one comment can garner tens of times the number of likes than the number of comments. So, if we do not take into account the likes, we might miss the moment when one negative or positive comment could be supported by thousands more people. We therefore carried out a weighted sentiment analysis and found that the score of the sentiment analysis increased to 0.61, meaning that people were more likely to support positive comments.

In order to understand the main topics discussed by the viewers in the comments, a topic modelling analysis was performed. As a result of the analysis and subsequent validation, comments were divided into 4 categories: Personal Experience, Opinion, Praise, Spam. In other words, people most often shared their own experiences, expressed their opinions with theorisations, explanations, critcisms of asexuality, aromanticism and other related identities, praised the video and expressed their gratitude, or wrote irrelevant comments or comments in another language. In contrast to research \citep{Tanvi2019}, we found that people quite often shared their own experiences rather than telling stories on behalf of a "friend". But of the commonalities with article \citep{Tanvi2019}, we found also comments that could be seen as declarations or general statements rather than experiences per se.
...
Nowadays, most people spend a lot of time on social media, and consider Youtube as a project where one can gain general knowledge about various topics. Our research demonstrated how one could measure audience attitudes towards videos revealing new topics, including understanding the 'mood' of the audience, and learning about the topics being discussed in the comments. 

<!-- Found an example of this paragraph in \citep{Tanvi2019} -->

# Ethical considerations

As we were working with comments, we were dealing with the text of specific people, so there is an ethical aspect that needed to be considered. We used the available Youtube API, in which the developers have preliminarily limited the set of parameters that can be collected from users on that site. Thus, we did not collect user data that the user did not consent to. Moreover, in our work we anonymised all comments by removing the \emph{authorChannelId} at the beginning.

# Elevator Pitch
<!-- End your notebook with an elevator pitch: 3-5 sentences summarizing the core of your project, design and its conclusion.-->
