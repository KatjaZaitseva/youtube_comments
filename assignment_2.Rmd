...
# followin code is adapted from lecture 5
install.packages("vader")
library(vader)
#I first sampled 100 comments, since it took too long to run
sample_cleaned <- sample_n(cleaned, 100)
#next I used the vade_df function
vader_results<-vader_df(sample_cleaned$textCleaned)
vader_results
mean_vader<-mean(vader_results$compound)
#the mean is positive, so more positive comments
mean_vader

#graphical representation of compound scores
hist(vader_results$compound)
```
```{r sentiment analysis}
#next, I also wanted to try out a different approach with lists of positive and nevative words
# followin code is adapted from lecture 5
...
#Select only random 100 reviews to speed up analysis
dic_scores = sample_cleaned$textCleaned %>% 
  corpus()%>%
  tokens() %>% 
  dfm() %>% 
...
#the mean is positive, so more positive comments
mean_dic<-mean(dic_scores$sent)
mean_dic

#graphical representation of compound scores
hist(dic_scores$sent)
```
